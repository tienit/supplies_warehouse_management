...
namespace VKTIM
{
    public partial class frmDmOriginPopup : CommonForm
    {
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            DMORIGINInfo unitInfo = new DMORIGINInfo();
            unitInfo.ORIGIN_NAME = txt_data_name.Text;
            unitInfo.ORIGIN_DESCRIPTION = txt_data_description.Text;

            if (_ACTION_TYPE == ACTION_TYPE.AddNew)
            {
                try
                {
                    unitInfo = DMORIGINController.Instance().Insert(unitInfo);
                    if (unitInfo != null)
                    {
            ...
            else if (_ACTION_TYPE == ACTION_TYPE.Update)
            {
                try
                {
                    ...
                    DMORIGINController.Instance().Update(unitInfo);
                    _OBJECT_ID = unitInfo.ID;
                    //------ Add log -------
        ...
        private void frmDmOriginPopup_Load(object sender, EventArgs e)
        {
            try
            {
                if (_ACTION_TYPE == ACTION_TYPE.AddNew)
