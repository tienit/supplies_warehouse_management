...
namespace VKTIM
{
    public partial class frmDmRank : CommonFormAdmin
    {
        public frmDmRank()
        {
            InitializeComponent();
        ...
        protected override void Init_Components()
        {
            ...
            _ARR_FILE_NAME.Add(new GridLayoutInfo("RANK_NAME", GIRD_COLUMN_TYPE.TextBox));
            _ARR_FILE_NAME.Add(new GridLayoutInfo("RANK_DESCRIPTION", GIRD_COLUMN_TYPE.TextBox));
        }

        protected override void Do_Add()
        {
            frmDmRankPopup m_Popup = new frmDmRankPopup();
            m_Popup.Text = "Thêm cấp bậc";
        ...
        protected override void Do_Update()
        {
            if (GRID_DATA.SelectedRows.Count > 0)
            {
                ...
                frmDmRankPopup m_Popup = new frmDmRankPopup();
                m_Popup.Text = "Cập nhật cấp bậc";
                m_Popup.ACTION_TYPE = ACTION_TYPE.Update;
                m_Popup.txt_data_id.Text = dr.Cells["col_ID"].Value.ToString();
                m_Popup.txt_data_name.Text = (string)dr.Cells["col_RANK_NAME"].Value;
                m_Popup.txt_data_description.Text = (dr.Cells["col_RANK_DESCRIPTION"].Value != null) ? dr.Cells["col_RANK_DESCRIPTION"].Value.ToString() : "";
                if (m_Popup.ShowDialog() == DialogResult.OK)
                {
                    Load_Data();
                }
        ...
        protected override void Do_Delete()
        {
            if (GRID_DATA.SelectedRows.Count > 0)
            {
                if (GBTSCCommon.Message_Confirm(GBTSCConstants.MSG_DELETE_CONFIRM, GBTSCConstants.MSG_CAPTION_DELETE) == DialogResult.Yes)
                {
                    ...
                    //Delete
                    DMRANKController.Instance().Delete(objID);
                }
            }
        }
        ...
        protected override void Do_Search()
        {
            ...
            DataTable dt = DMRANKController.Instance().Search_DS(KEY_WORD);
