...
namespace VKTIM
{
    public partial class frmDmLocationPopup : CommonForm
    {
        ...
        public frmDmLocationPopup()
        {
            InitializeComponent();
        }

        private void frmDmLocationPopup_Load(object sender, EventArgs e)
        {
            try
            {
                if (_ACTION_TYPE == ACTION_TYPE.AddNew)
                {
                    txt_data_locationname.Focus();
                }
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            DMLOCATIONInfo unitInfo = new DMLOCATIONInfo();
            unitInfo.LOCATION_NAME = txt_data_locationname.Text;
            unitInfo.LOCATION_ORDER = int.Parse( txt_data_order.Text);
            unitInfo.LOCATION_PARENT_ID = int.Parse( txt_data_LocationParrentID.Text);
            unitInfo.LOCATION_LEVEL = int.Parse(txt_data_LocationLevel.Text);

            if (_ACTION_TYPE == ACTION_TYPE.AddNew)
            {
                try
                {
                    unitInfo = DMLOCATIONController.Instance().Insert(unitInfo);
                    if (unitInfo != null)
                    {
                        _OBJECT_ID = unitInfo.ID;
                        //------ Add log -------
                        DTLOGEVENTController.Instance().Insert(new DTLOGEVENTInfo(0, "LOCATION_ADD", "OK", "Thêm đơn vị thành công.", GBTSCCommon.SERVER_NOW(), GBTSCConstants.CURRENT_USER.ID, GBTSCConstants.CURRENT_USER.USER_NAME, Environment.UserName, Environment.MachineName, Environment.UserDomainName, Environment.CurrentDirectory, ""));
            ...
            else if (_ACTION_TYPE == ACTION_TYPE.Update)
            {
                try
                {
                    ...
                    DMLOCATIONController.Instance().Update(unitInfo);
                    _OBJECT_ID = unitInfo.ID;
                    //------ Add log -------
