...
namespace VKTIM
{
    public partial class frmDmSpecial : CommonFormAdmin
    {
        public frmDmSpecial()
        {
            InitializeComponent();
        ...
        {
            _TABLE_NAME = "DM_SPECIAL";
            _ARR_FILE_NAME = new List<GridLayoutInfo>();
            _ARR_FILE_NAME.Add(new GridLayoutInfo("ID", GIRD_COLUMN_TYPE.TextBox));
            _ARR_FILE_NAME.Add(new GridLayoutInfo("SPECIAL_NAME", GIRD_COLUMN_TYPE.TextBox));
            _ARR_FILE_NAME.Add(new GridLayoutInfo("SPECIAL_DESCRIPTION", GIRD_COLUMN_TYPE.TextBox));
        }

        protected override void Do_Add()
        {
            frmDmSpecialPopup m_Popup = new frmDmSpecialPopup();
            m_Popup.Text = "Thêm chuyên ngành";
        ...
        protected override void Do_Update()
        {
            if (GRID_DATA.SelectedRows.Count > 0)
            {
                ...
                frmDmSpecialPopup m_Popup = new frmDmSpecialPopup();
                m_Popup.Text = "Cập nhật chuyên ngành";
                m_Popup.ACTION_TYPE = ACTION_TYPE.Update;
                m_Popup.txt_data_id.Text = dr.Cells["col_ID"].Value.ToString();
                m_Popup.txt_data_name.Text = (string)dr.Cells["col_SPECIAL_NAME"].Value;
                m_Popup.txt_data_description.Text = (dr.Cells["col_SPECIAL_DESCRIPTION"].Value != null) ? dr.Cells["col_SPECIAL_DESCRIPTION"].Value.ToString() : "";
                if (m_Popup.ShowDialog() == DialogResult.OK)
                {
                    Load_Data();
                }
        ...
        protected override void Do_Delete()
        {
            if (GRID_DATA.SelectedRows.Count > 0)
            {
                if (GBTSCCommon.Message_Confirm(GBTSCConstants.MSG_DELETE_CONFIRM, GBTSCConstants.MSG_CAPTION_DELETE) == DialogResult.Yes)
                {
                    ...
                    //Delete
                    DMSPECIALController.Instance().Delete(objID);
                }
            }
        }
        ...
        protected override void Do_Search()
        {
            ...
            DataTable dt = DMSPECIALController.Instance().Search_DS(KEY_WORD);
